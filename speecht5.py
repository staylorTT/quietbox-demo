...
def run_on_tt():
    ...
    time_end = time.time()
    iter_times.append(time_end - time_start)
    print(f"ITER 1 TIME: {time_end - time_start} seconds")
    ...
    time_end = time.time()
    iter_times.append(time_end - time_start)
    print(f"ITER 2 TIME: {time_end - time_start} seconds")

    time_start = time.time()
    speech = _generate_speech(model, **inputs, speaker_embeddings=speaker_embeddings, vocoder=vocoder)
    time_end = time.time()
    iter_times.append(time_end - time_start)
    print(f"ITER 3 TIME: {time_end - time_start} seconds")

    for iter_time in iter_times:
        print(f"ITER TIME: {iter_time} seconds")
